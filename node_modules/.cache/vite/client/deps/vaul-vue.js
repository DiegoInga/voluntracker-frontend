import {
  ...
  ou,
  te
} from "./chunk-YHNISR6U.js";
import "./chunk-A2D2ZT3H.js";
import "./chunk-XPMVRGWO.js";
...
function $e(t, n, v, r = {}) {
  ...
  if (_) {
    ...
    return Y(
      () => t[n],
      (y) => {
        S || (S = true, m.value = U(y), Oe(() => S = false));
      }
    ), Y(
      m,
      (y) => {
...
function vt(t) {
  const {
    ...
    fadeFromIndex: p,
    direction: o
  } = t, U = E(b.value ?? false), C = E(false), s = E(false), l = E(false), m = E(false), S = E(null), y = E(null), x = E(null), M = E(null), j = E(null), X = E(true), K = E(null), z = E(0), F = E(false);
  E(0);
  const d = E(null);
  E(0);
  const L = W(() => {
  ...
  function He(a) {
    !u.value && !B.value || d.value && !d.value.$el.contains(a.target) || (l.value = true, x.value = /* @__PURE__ */ new Date(), a.target.setPointerCapture(a.pointerId), z.value = P(o.value) ? a.clientY : a.clientX);
  }
  function Ue(a) {
    var g, h, N, R, H, V;
    if (d.value && l.value) {
      const J = o.value === "bottom" || o.value === "right" ? 1 : -1, ee = (z.value - (P(o.value) ? a.clientY : a.clientX)) * J, te2 = ee > 0, we = B.value && !u.value && !te2;
      if (we && fe.value === 0)
        return;
      const se = Math.abs(ee), he = document.querySelector("[vaul-drawer-wrapper]");
      let I = se / L.value;
      const be = Ce(se, te2);
      if (be !== null && (I = be), we && I >= 1 || !X.value && !me(a.target, te2))
        return;
      if ((g = d == null ? void 0 : d.value) == null || g.$el.classList.add(Se2), X.value = true, $((h = d.value) == null ? void 0 : h.$el, {
        transition: "none"
      }), $((N = S.value) == null ? void 0 : N.$el, {
        transition: "none"
      }), B.value && _e({ draggedDistance: ee }), te2 && !B.value) {
  ...
  function Q() {
    ...
    d.value && (r(), $(d.value.$el, {
      ...
      opacity: "0",
      transition: `opacity ${w.DURATION}s cubic-bezier(${w.EASE.join(",")})`
    }), ue(false), window.setTimeout(() => {
      s.value = false, U.value = false;
  ...
  function Me(a) {
    if (!l.value || !d.value)
      return;
    d.value.$el.classList.remove(Se2), X.value = false, l.value = false, M.value = /* @__PURE__ */ new Date();
    const g = le(d.value.$el, o.value);
    if (!me(a.target, false) || !g || Number.isNaN(g) || x.value === null)
      return;
    const h = M.value.getTime() - x.value.getTime(), N = z.value - (P(o.value) ? a.clientY : a.clientX), R = Math.abs(N) / h;
    if (R > 0.05 && (m.value = true, window.setTimeout(() => {
      m.value = false;
    }, 200)), B.value) {
  ...
  Y(U, (a) => {
    a && (y.value = /* @__PURE__ */ new Date(), ue(true)), e(a);
  }, { immediate: true }), Y(b, (a) => {
    a ? (U.value = a, C.value = true) : Q();
  }, { immediate: true });
  function ue(a) {
...
var wt = oe({
  ...
  setup(t) {
    ...
    function U(C) {
      ...
      i.value && (i.value = false), C.preventDefault(), c.value && T(false), !(!c.value || n.value !== void 0) && D();
    }
    return Y(
      v,
      (C) => {
